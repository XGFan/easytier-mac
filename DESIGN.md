...
本文件是 supervisor / 安装脚本 / 验证脚本 / GUI 之间的接口契约,涉及路径/协议/生命周期的改动需先同步本文档。(原始计划书存于 EasyTier fork 仓库的 `.omc/plans/easytier-mac-gui-tray-supervisor.md`,仅历史参考。)
...
| crate | `supervisor/`,package `easytier-supervisor`(本仓库 workspace member) |
...
- `--daemon` 已实证不 fork/detach(vendor/EasyTier/easytier/src/core.rs:1375 仅注册 DaemonGuard),子进程跟踪成立;此假设写进代码注释。
...
`scripts/hooks-examples/{up.sh,down.sh}` 提供一份 DNS 切换示例(连接时把 Wi-Fi/有线等所有启用中的网络服务 DNS 设为目标地址,断开时恢复 DHCP),含安装方法(`sudo cp` + `chown root:wheel` + `chmod 755`)与幂等写法,可直接参考或改造。
...
- **目录**:`app/`(SwiftUI 菜单栏应用)+ `bridge/`(cdylib,cargo workspace member);`gui/`(Tauri)已在功能对等验收后整体删除(历史契约存档于 §8)。
- **配置**:唯一 `~/Library/Application Support/EasyTier/config.toml`,GUI 只读;仅文件缺失时写入一次带注释模板(模板即格式文档,含常用字段说明);**不迁移** profiles/ 存量。校验 = `TomlConfigLoader` + `NetworkConfig::new_from_config` 双层解析。面板打开时重读重校;连接前必校,失败禁用连接;连接中文件与运行中配置不一致 → 提示「配置已修改,重新连接后生效」。
...
- **UI**:无主窗口,全部界面收进菜单栏面板(MenuBarExtra `.window` 样式,宽 430,无箭头)。菜单栏图标 = 单色模板网格 glyph,已连接实心、断开空心,不用彩色。主页面自上而下:状态 pill(点击即连接/断开,四态文案 已连接/连接中…/断开中…/已断开,过渡态、校验失败、未安装 supervisor 时禁点)+「校验不通过」徽标(完整错误摘要在 hover tooltip)+ 设置与退出按钮 → 消息行(statusMessage / 「配置已修改,重新连接后生效」,仅有内容时出现)→ 速率图(轮询 rx/tx 差分)→ 节点表(节点[主机名+虚拟IPv4]/延迟/下载/上传/丢包/NAT,本机行置顶;路由/协议/版本收进行 hover tooltip,无列自定义)。设置页为面板内二级导航(‹ 返回):开机自启/启动时自动连接/自动重启 + 配置文件(打开/Finder)+ Supervisor 卸载 + 版本。未安装 supervisor 时主页面内容区显示引导卡。连接期间状态轮询常驻,与面板可见性无关。不做 VPN 门户与事件日志。单实例、busy/takeover 交互沿用 §8。
